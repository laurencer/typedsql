...
object HiveType {
  object parsers {
    ...
    def decimalWithValuesParser = P("decimal(" ~ CharsWhile(_.isDigit).! ~ "," ~ CharsWhile(_.isDigit).! ~ ")").map({ case (precision, scale) => DecimalType(precision.toInt, scale.toInt) })
    ...
    def mapParser = P("map<" ~ hiveType ~ "," ~ hiveType ~ ">").map({ case (key, value) => MapType(key, value)})
    def arrayParser = P("array<" ~ hiveType ~ ">").map(ArrayType(_))

    def fieldParser: Parser[(String, HiveType)] = P(fieldIdentifier ~ ":" ~ hiveType)
    def structParser = P("struct<" ~ fieldParser.rep(sep = ",") ~ ">").map(ls => StructType(ls.toList))

    def hiveType: Parser[HiveType] = scalarTypes | mapParser | arrayParser | structParser
  }
