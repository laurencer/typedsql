      ...
      .fold(Map.empty[FieldName, DefaultValue])(_.map({
        case q"$mods val ${name}: ${objectType}" => {
          ...
          val sqlParameterImplicit = try {
            ...
            case ex: TypecheckException => c.abort(c.enclosingPosition, s"SqlParameter[${objectType}] implicit could not be found and is required for the parameter ${name}.")
          }
          ...
          val defaultValue = tpe match {
            ...
            case other                        => {
              println(s"Encountered unknown SqlParameter[${objectType}] - using an empty string as a placeholder value...")
              "\"\""
            }
          }
