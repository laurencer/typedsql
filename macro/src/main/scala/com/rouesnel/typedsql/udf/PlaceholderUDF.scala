...
object UdfDescription {
  ...
  implicit def jsonCodec = casecodec3(UdfDescription.apply _, UdfDescription.unapply _)(
    "name",
    "arguments",
    "return_type"
  )
...
abstract class PlaceholderUDF(val placeholderIndex: Int) extends GenericUDF {
  val description: UdfDescription = {
    PlaceholderUDF.udfs
      ...
      .getOrElse(throw new Exception(s"Could not find UDF placeholder for index ${placeholderIndex}"))
  }

  override def initialize(arguments: Array[ObjectInspector]): ObjectInspector = {
    arguments.toList.zip(description.arguments).zipWithIndex.foreach({
      case ((argumentType, (_, expectedType)), idx) =>
        if (argumentType.getTypeName == expectedType.hiveType) {

        } else {
          throw new UDFArgumentTypeException(idx, s"Wrong type given to ${getUdfName()} - expected ${expectedType.hiveType} but instead got ${argumentType.getTypeName}.")
        }
    })

    description.returnType match {
      case pt: PrimitiveType => pt match {
        case IntType => PrimitiveObjectInspectorFactory.javaIntObjectInspector
        case StringType => PrimitiveObjectInspectorFactory.javaStringObjectInspector
      }
      case other => throw new UDFArgumentTypeException(0, s"Unsupported return type: ${other.hiveType}")
    }
  }

  override def getDisplayString(children: Array[String]): String = s"Placeholder for ${description.name}"
...
object PlaceholderUDF {
  ...
  val placeholders = 0.to(numberOfPlaceholders).toList.map(idx =>
    (udf: UdfDescription) => {
      udfs.put(idx, udf)
      getClass
        .getClassLoader
  ...
  def configurePlaceholders[T](udfs: List[UdfDescription])(f: (UdfDescription, Class[PlaceholderUDF]) => T): String \/ List[T] = {
    if (udfs.size > numberOfPlaceholders) {
      val prettyUdfs = udfs.map(udf => s"- ${udf.name}").mkString("\n")
      \/.left(s"Only ${numberOfPlaceholders} UDFs are supported and ${udfs.size} were given:\n${prettyUdfs}")
    } else {
      \/.right(udfs.zip(placeholders).map({
