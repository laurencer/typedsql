...
class ScroogeGenerator[Context <: whitebox.Context](val c: Context) {
  ...
  def generateStruct(structNamer: StructType => TypeName, struct: StructType): List[c.Tree] = {
    ...
    val thriftFields = struct.fields.toList.zipWithIndex.flatMap({ case ((fieldName, fieldType), idx) =>
      List(q"""
              val ${TermName(HiveType.camelCaseFieldName(fieldName).capitalize + "Field")} = new org.apache.thrift.protocol.TField(${Literal(Constant(fieldName))}, ${hiveTypeToThriftTypeName(fieldType)}, ${Literal(Constant(idx + 1))})""",
        q"""val ${TermName(HiveType.camelCaseFieldName(fieldName).capitalize + "FieldManifest")} = implicitly[Manifest[${resolveType(structNamer, fieldType)}]]""")
    })
      ...
      q"""
        case class ${generatedName}(..$fields) extends com.twitter.scrooge.ThriftStruct {
          override def write(oprot: org.apache.thrift.protocol.TProtocol): Unit = {
            throw new scala.NotImplementedError("TypedSQL does not support writing the generated structs (" + ${Literal(Constant(generatedName.toString))} + ")")
          }
        }
       """,
      q"""
       object ${generatedName.toTermName} extends com.twitter.scrooge.ThriftStructCodec3[${generatedName}] {
         ...
         override def encode(t: ${generatedName}, oprot: org.apache.thrift.protocol.TProtocol): Unit = {
           throw new scala.NotImplementedError("TypedSQL does not support encoding fields in generated structs (" + ${Literal(Constant(generatedName.toString))} + ")")
         }
   ...
   */
  def buildThriftDecode(structNamer: StructType => TypeName, structName: TypeName, fields: List[(String, HiveType)]) = {
    ...
    val readMethods = fields.map({ case (fieldName, hiveType) =>
      q"""private def ${readMethodName(fieldName)}(_iprot: org.apache.thrift.protocol.TProtocol): ${resolveType(structNamer, hiveType)} = {
