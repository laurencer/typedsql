      ...
      (config: Config, underlying: PersistableSource[T]) => {
        ...
        Execution.from {
          ...
          val alreadyExists = HiveMetadataTable
            .existsTableStrict[T](db, tableName, underlying.partitions, Some(absolutePath))
            .run(config.conf)
            .fold[Boolean](
              identity _,
              err =>
      ...
      .flatMap(_ =>
        Execution.from {
          DataSource.synchronized {
            HiveMetadataTable
              .createTable[T](db, table, Nil, Some(new Path(path)))
          ...
          """.stripMargin
            ...
            if (response.getResponseCode() != 0)
              ...
              databaseName.foreach(db => {
                ...
                if (response.getResponseCode() != 0)
                  throw new Exception(
                    s"Error selecting database ${db}. ${response.getErrorMessage}")
              })
