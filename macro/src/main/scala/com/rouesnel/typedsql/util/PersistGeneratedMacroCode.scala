  ...
  def apply[C <: Context](c: C)(code: String): Unit = {
    ...
    Option(c.enclosingPosition.source.file.file) match {
      case None =>
        c.warning(
          c.enclosingPosition,
          "Could not persist macro-generated output because the source file location is not available.")
      case Some(src) => {
        ...
        c.info(c.enclosingPosition,
               s"Macro generated code can be found at ${persisted.getAbsoluteFile}",
               false)
