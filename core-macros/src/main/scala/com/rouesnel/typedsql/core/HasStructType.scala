...
object HasStructType {
  implicit def materialiseHasStructType[T <: ThriftStruct]: HasStructType[T] =
    macro impl[T]
  ...
  def impl[T](c: Context)(implicit typ: c.WeakTypeTag[T]): c.Expr[HasStructType[T]] = {
    ...
    val result =
      q"com.rouesnel.typedsql.core.HasStructType[${typ}](com.rouesnel.typedsql.core.HiveType.parseHiveType(${Literal(
        Constant(hiveType.hiveType))}).toOption.get.asInstanceOf[com.rouesnel.typedsql.core.StructType])"
